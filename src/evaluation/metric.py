...
def compute_metrics_with_confusion(y_true, y_pred, y_prob):
    ...
    return {
        ...
        "specificity": spec,
        "ppv": ppv,
        "precision": ppv,      # ✅ 추가: precision alias for ppv
        "npv": npv,
        "f1": f1,
        "tp": int(tp),
        "tn": int(tn),
...
def print_confusion_matrix_summary(fold_results):
    ...
    total_tp = sum(r['test_metrics']['tp'] for r in fold_results)
    total_tn = sum(r['test_metrics']['tn'] for r in fold_results)
    total_fp = sum(r['test_metrics']['fp'] for r in fold_results)
    total_fn = sum(r['test_metrics']['fn'] for r in fold_results)
...
def plot_metric_comparison(fold_results, save_dir):
    ...
    for idx, (metric, name) in enumerate(zip(metrics, metric_names)):
        ...
        test_values  = [safe_get(r['test_metrics'], metric) for r in fold_results]
