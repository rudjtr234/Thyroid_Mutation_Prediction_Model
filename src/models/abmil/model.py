...
class ABMIL(MIL):
    ...
    Args:
        in_dim (int): Input feature dimension for each instance (default: 1536).
        ...
        gate (bool): Whether to use gated attention (True) or standard attention (False) (default: True).
    ...
    def forward(self, h: torch.Tensor,
                ...
                return_extra: bool = False) -> torch.Tensor:
...
class ABMILGatedBaseConfig(PretrainedConfig):
    """
    Configuration class for the ABMIL Gated Base model for UNI2 features.

    This class stores the configuration parameters required to instantiate an ABMIL model
    with gated attention optimized for UNI2 embeddings (1536-dim).
    ...
    Args:
        ...
        num_fc_layers (int): Number of fully connected layers in the patch embedding MLP (default: 1).
        dropout (float): Dropout rate applied in the MLP and attention layers (default: 0.25).
        in_dim (int): Input feature dimension for each instance (default: 1536 for UNI2).
        num_classes (int): Number of output classes for the classification head (default: 2).
...
if __name__ == "__main__":
    print("Testing UNI2 ABMIL for BRAF classification...")
    
    # Create model with UNI2 dimensions
    config = ABMILGatedBaseConfig()  # Uses UNI2 defaults
    model = ABMILModel(config)
    
    print(f"Model parameters: {sum(p.numel() for p in model.parameters()):,}")
    
    # Test with dummy UNI2 features
    batch_size = 1
    num_patches = 1000
    uni2_dim = 1536
    
    dummy_features = torch.randn(batch_size, num_patches, uni2_dim)
    dummy_label = torch.tensor([1])  # BRAF+
