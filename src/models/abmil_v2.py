...
class ABMIL(MIL):
    ...
    def __init__(self,
                 gate: bool = True,
                 embed_dim: int = 768,         # 512 → 768 (절충)
                 attn_dim: int = 512,          # 384 → 512 (절충)
                 num_fc_layers: int = 2,       # 1 → 2 (gradual projection)
                 dropout: float = 0.25,         # 0.25 → 0.5 (과적합 방지 강화)
        ...
        # Feature normalization (UNI2 고차원 feature 안정화)
    ...
    def forward_attention(self, h: torch.Tensor, attn_mask=None, attn_only=True) -> torch.Tensor:
        ...
        """
        # Input normalization (UNI2 feature 안정화)
...
class ABMILGatedBaseConfig(PretrainedConfig):
    """
    Configuration class for UNI2-optimized ABMIL model.
    Balanced design: ~2.5M parameters for 1,461 training bags.
    """
    model_type = MODEL_TYPE

    def __init__(self,
                 gate: bool = True,
                 embed_dim: int = 768,         # 절충안
                 attn_dim: int = 512,          # 절충안
                 num_fc_layers: int = 2,
                 dropout: float = 0.25,         # 강화
                 in_dim: int = 1536,
                 num_classes: int = 2,
                 use_layer_norm: bool = True,
                 **kwargs):
...
if __name__ == "__main__":
    print("Testing UNI2-optimized ABMIL (Balanced) for BRAF classification...")
    print("=" * 60)

    # Create model with balanced config
    ...
    print(f"  - Params/Sample ratio: ~{sum(p.numel() for p in model.parameters())/1461:.1f} (for 1,461 bags)")
    print()
    ...
    print()
    print("UNI2-optimized ABMIL (Balanced) working correctly!")
    print("=" * 60)
