...
# 경로 설정: models 디렉토리를 sys.path에 추가 (모듈 로딩 시 1회만 실행)
...
def upload_to_mlflow(
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        mlflow.set_tag(
            "Description",
            "WSI → Patch(512×512) → UNI2 Embedding → ABMIL → BRAF Mutation Classification. "
            "5-fold Stratified CV(8:1:1). Model: ABMIL + UNI2(1536-dim).",
        )
        ...
        if model_checkpoint_path:
            try:
                # Checkpoint 로드 및 구조 확인
                ckpt = torch.load(model_checkpoint_path, map_location="cpu", weights_only=False)

                # Checkpoint에서 모델 구조 추론
                state_dict = ckpt["model_state_dict"]

                # Key naming에서 prefix 확인
                has_model_prefix = any(k.startswith('model.') for k in state_dict.keys())

                # patch_embed의 첫 레이어 weight shape으로 embed_dim 추론
                embed_key = "model.patch_embed.0.weight" if has_model_prefix else "patch_embed.0.weight"
                if embed_key in state_dict:
                    embed_dim = state_dict[embed_key].shape[0]
                else:
                    embed_dim = 768  # 기본값

                # attention_V weight shape으로 attn_dim 추론
                attn_key = "model.global_attn.attention_V.0.bias" if has_model_prefix else "global_attn.attention_V.0.bias"
                if attn_key in state_dict:
                    attn_dim = state_dict[attn_key].shape[0]
                else:
                    attn_dim = 512  # 기본값

                # 추론된 구조로 모델 생성
                from models.abmil_v2 import ABMIL
                model = ABMIL(
                    gate=True,
                    embed_dim=embed_dim,
                    attn_dim=attn_dim,
                ...
                # State dict 로드 (model. prefix 제거)
                # Note: state_dict already loaded at line 343, just remove prefix if needed
                if has_model_prefix:
                ...
                if bag_size is not None:
                    ...
                    # Full NPY 모드: 안전한 기본값 사용
                    # ABMILModel은 가변 길이를 처리하므로 signature는 대표값으로 충분
                    num_patches = 2000

                dummy = torch.randn(1, num_patches, 1536)
                output = model(h=dummy, return_extra=True)
                logits = output["logits"]
                if logits.ndim == 1:
                    logits = logits.unsqueeze(0)

                signature = infer_signature(dummy.numpy(), logits.detach().numpy())
                ...
                mlflow.register_model(
                    ...
                    name=f"braf_abmil_{version.replace('.', '_')}"
                )
