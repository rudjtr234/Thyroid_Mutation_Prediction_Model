...
class ThyroidWSIDataset(Dataset):
    ...
    """
    def __init__(self, wsi_files, bag_size=2000, use_variance=False):  # ← 들여쓰기 4칸!
        """
        Args:
        ...
        for filepath in wsi_files:
            ...
            # 새로 추가: preprocess_data 경로 지원
                ...
                label = 1  # BRAF+
            elif 'braf_non_meta' in filepath or 'nonmeta' in filepath:
                label = 0  # BRAF-
            else:
                # 파일명으로 추론 불가능한 경우 경고
    ...
    def _select_tiles(self, features):  # ← 들여쓰기 4칸!
        """타일 선택: Random (use_variance=False)"""
        # bag_size가 None이면 전체 features 반환 (test set용)
    ...
    def __len__(self):  # ← 들여쓰기 4칸!
        return len(self.wsi_list)

    def __getitem__(self, idx):  # ← 들여쓰기 4칸!
