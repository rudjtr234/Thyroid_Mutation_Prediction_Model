...
class ThyroidBagDataset(Dataset):
    def __init__(self, wsi_files, bag_size=2000, use_variance=False):
        ...
        for filepath in wsi_files:
            label = 1 if '/meta/' in filepath else 0
            features = np.load(filepath)

            # 여러 bag으로 쪼개기
            num_bags = int(np.ceil(len(features) / bag_size))
            for b in range(num_bags):
                start = b * bag_size
                end = min((b + 1) * bag_size, len(features))
                bag_feats = features[start:end]

                if len(bag_feats) < bag_size:
                    continue  # 작은 bag은 제외 (원하면 zero-padding 가능)

                self.bags.append({
                    "features": bag_feats,
                    "label": label,
                    "wsi_name": os.path.basename(filepath),
                    "bag_idx": b
                })
    ...
    def __getitem__(self, idx):
        ...
        return features, label, bag["wsi_name"], bag["bag_idx"]
